...
After loading the package, currently there is only one new function: `factory_geom_line()`.
This is our *geom_factory* that will create new `geom_line()` variations that will work on a alternative color scale.
The function also creates according scales `scale_*()` (one discrete, one continuous) as well as the needed color-bar guide `guide_colourbar_*()`.
...
First, we create three data sets that all refer to the same spatial system (in this case our simple x axis), but which are otherwise unrelated:
...
ggplot(data1, aes(x,y))+
  ...
  geom_line_var3(data = data3, aes(var3 = var3))+
  scale_var3_manual(values = RColorBrewer::brewer.pal(3,'Set1')) +
